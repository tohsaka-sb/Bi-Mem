...
def simple_tokenize(text):
    """Tokenize text for BM25. Uses regex to avoid NLTK (no network download needed)."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    delay = 5 * (2 ** attempt)
                    print(f"OpenAI API timeout (attempt {attempt + 1}/{max_retries}). Retrying in {delay}s...")
                    time.sleep(delay)
                else:
                    raise
...
'''
    ...
    def find_related_memories(self, query: str, k: int = 5, strategy: str = "graph_traversal", top_k_levels: List[int] = [2, 3, 5], include_scene_insights: bool = True) -> (str, List[str]):
        """
        ...
        Args:
            ...
            include_scene_insights: If False, scene-level node content is excluded from the returned context string.
        """
