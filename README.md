...
Use scripts/eval/test_advanced.py to run the full pipeline (Memory Construction -> Hierarchy Building -> QA Evaluation).
...
4. Utilities
scripts/analysis/extract_results.py
scripts/maintenance/clean_hierarchy.py: Delete Level 1/2 nodes to force hierarchy reconstruction.
scripts/maintenance/inspect_memory.py: View the content of the constructed memory graph.


python scripts/maintenance/inspect_memory.py path/to/memory_cache.pkl
📂 Project Structure
src/bimem/core/memory_layer.py: Core logic for AgenticMemorySystem, Graph operations, and Retrieval.
src/bimem/core/memory_managers.py: SceneManager and personaManager classes (Prompts & LLM interactions).
scripts/eval/test_advanced.py: Main execution script, QA logic, and Evaluation loop.
src/bimem/core/load_dataset.py: Data loader for LoCoMo dataset.
src/bimem/core/utils.py: Metric calculation (F1, BLEU, BERTScore).
📊 Performance (Sample)
