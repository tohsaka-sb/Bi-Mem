    ...
    def answer_question(self, question: str, category: int, answer: str) -> str:
        ...
        """
            ...
            if is_choice:
                ...
                2. Answer with the chosen option. You MAY add a brief reason after ";".
                3. Examples: "National park" or "National park; she likes the outdoors".
                """
            elif is_yes_no:
                cat3_instruction = """
                1. Answer "Yes", "No", "Likely", "Likely no", "Unlikely", or "Somewhat" (for nuanced cases).
                2. When the answer is "probably not", prefer **"Likely no"** over "Unlikely" to match common format.
                3. You MAY add a brief reason after ";" or ",". Keep it concise, third person (e.g., "she is supportive").
                4. Examples: "Yes; she loves art.", "Likely no; since this one went badly", "No; she's in the process of adopting."
                """
            else:
                cat3_instruction = """
                1. Analyze personality and history to infer. Use context clues. Avoid "Not specified".
                2. Give a **very short phrase** (1-5 words or short comma list). Match reference concision.
                3. Examples: "Liberal", "Thoughtful, authentic, driven". No full sentences.
                """
...
def _refine_fact_memories(agent, save_callback=None):
    """
    瀵?Level 0 (Fact) 灞傝蹇嗚繘琛?Top-Down Refinement锛屼笌 Scene 灞備娇鐢ㄧ浉鍚岀殑 refinement_prompt銆?    鍩轰簬 Persona 鐢诲儚涓烘瘡涓?Fact 杩藉姞涓庣敾鍍忕浉鍏崇殑 insight銆?    """
    memory_system = agent.memory_system
    graph = memory_system.graph

    level2_nodes = [n for n, d in graph.nodes(data=True) if d.get('level') == 'persona']
    if not level2_nodes:
        print("Skipping fact refinement: No Level 2 (Persona) nodes available.")
        return

    user_profile_text = ""
    for nid in level2_nodes:
        node_attrs = graph.nodes[nid]
        tag = node_attrs.get('tags', ['Unknown'])[0]
        content = node_attrs.get('content', '')
        user_profile_text += f"[{tag}]: {content}\n"

    level0_nodes = [n for n, d in graph.nodes(data=True) if d.get('level') == 'fact']
    facts_to_refine = [nid for nid in level0_nodes if not graph.nodes[nid].get('refined', False)]
    total_facts = len(level0_nodes)
    processed_count = total_facts - len(facts_to_refine)

    print("\n--- Starting Fact Layer Refinement (Level 2 -> Level 0) ---")
    print(f"Found {processed_count} already refined facts.")
    print(f"Refining {len(facts_to_refine)} remaining facts...")

    updated_count = 0
    CHECKPOINT_INTERVAL = 10

    with tqdm(total=total_facts, initial=processed_count, desc="Refining Facts") as pbar:
        for fact_id in facts_to_refine:
            current_content = graph.nodes[fact_id].get('content', '')

            if new_content:
                graph.nodes[fact_id]['content'] = new_content
                memory_system._update_retriever([fact_id])
                updated_count += 1

            graph.nodes[fact_id]['refined'] = True
            pbar.update(1)

            if save_callback and updated_count % CHECKPOINT_INTERVAL == 0:
                save_callback()

    print(f"Fact refinement complete. Updated {updated_count} facts.")
